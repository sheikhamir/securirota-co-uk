...
try {
    ...
    $stmt = $conn->query("SELECT MONTH(shift_date) as month, 
                                 SUM(
                                     CASE 
                                         WHEN end_time >= start_time THEN TIME_TO_SEC(TIMEDIFF(end_time, start_time))/3600
                                         ELSE TIME_TO_SEC(TIMEDIFF(end_time, start_time))/3600 + 24
                                     END
                                 ) as total_hours
                          FROM shifts 
    ...
    foreach ($monthly_data as $data) {
        if (isset($data['month']) && $data['month'] >= 1 && $data['month'] <= 12) {
            $monthly_hours[$data['month'] - 1] = round((float)$data['total_hours'], 1);
        }
    }
    
    // Recent critical activities (last 48 hours) - using correct column names
    $stmt = $conn->query("SELECT al.*, u.username as user_name FROM activity_log al 
                          LEFT JOIN users u ON al.user_id = u.id
                          WHERE al.action_type IN ('login', 'logout', 'create_user', 'delete_user') 
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h5 class="card-title mb-0">
                                    <i class="fas fa-chart-line"></i> Monthly Hours Overview
                                </h5>
                            </div>
                            <div class="card-body">
                                            ...
                                            <div class="d-flex justify-content-between align-items-start">
                                                <div class="flex-grow-1">
                                                    <div class="d-flex align-items-center mb-1">
                                                        <strong><?php echo htmlspecialchars($activity['action_type'] ?? 'Unknown'); ?></strong>
                                                        <span class="badge bg-secondary ms-2"><?php echo htmlspecialchars($activity['user_name'] ?? $activity['user_id'] ?? 'System'); ?></span>
                                                    </div>
    ...
    <script>
        ...
        const monthlyChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Total Hours',
                    data: [<?php echo implode(',', $monthly_hours); ?>],
                    borderColor: '#667eea',
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                return 'Total Hours: ' + context.parsed.y;
                            }
                        }
                    }
                },
