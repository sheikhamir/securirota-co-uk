...
<head>
    ...
    <title><?php echo $page_title; ?> - Cache Management</title>
...
<body>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h1 class="h3 text-dark mb-0">
                                    <i class="fas fa-sync-alt text-primary"></i> Cache Management
                                </h1>
                                <p class="text-muted mb-0">Clear cache, optimize performance, and manage system storage</p>
                            </div>
                ...
                <div class="row mb-4">
                    <div class="col-12">
                        <h4 class="text-dark mb-3">
                            <i class="fas fa-tasks"></i> Cache Actions
                        </h4>
                    </div>
                    <div class="col-md-3 mb-3">
                        <div class="card quick-action-card">
                            <div class="card-body text-center">
                                ...
                                <p class="card-text text-muted">Clear all system cache files and temporary data</p>
                                <button class="btn btn-primary" onclick="clearCache('all')">Clear All</button>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3 mb-3">
                        <div class="card quick-action-card">
                            <div class="card-body text-center">
                                ...
                                <h5 class="card-title">Clear File Cache</h5>
                                <p class="card-text text-muted">Clear only file-based cache and temporary files</p>
                                <button class="btn btn-success" onclick="clearCache('files')">Clear Files</button>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3 mb-3">
                        <div class="card quick-action-card">
                            <div class="card-body text-center">
                                ...
                                <h5 class="card-title">Clear DB Cache</h5>
                                <p class="card-text text-muted">Clear database query cache and optimize tables</p>
                                <button class="btn btn-warning" onclick="clearCache('database')">Clear DB Cache</button>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3 mb-3">
                        <div class="card quick-action-card">
                            <div class="card-body text-center">
                                ...
                                <h5 class="card-title">Optimize System</h5>
                                <p class="card-text text-muted">Run full system optimization and cleanup</p>
                                <button class="btn btn-info" onclick="optimizeSystem()">Optimize</button>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3 mb-3">
                        <div class="card quick-action-card">
                            <div class="card-body text-center">
                                <div class="action-icon bg-secondary-gradient text-white mx-auto">
                                    <i class="fas fa-trash"></i>
                                </div>
                                <h5 class="card-title">Clean Logs</h5>
                                <p class="card-text text-muted">Clean old log files and error logs</p>
                                <button class="btn btn-secondary" onclick="cleanLogs()">Clean Logs</button>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3 mb-3">
                        <div class="card quick-action-card">
                            <div class="card-body text-center">
                                ...
                                <h5 class="card-title">Refresh Config</h5>
                                <p class="card-text text-muted">Reload configuration files and settings</p>
                                <button class="btn btn-danger" onclick="refreshConfig()">Refresh</button>
                            </div>
                        </div>
                    </div>
                </div>
                                            ...
                                            <div class="d-flex justify-content-between align-items-start">
                                                <div class="flex-grow-1">
                                                    <div class="d-flex align-items-center mb-1">
                                                        <strong><?php echo htmlspecialchars($activity['action'] ?? 'Unknown Action'); ?></strong>
    ...
    <script>
        function clearCache(type) {
            const confirmMessage = `Are you sure you want to clear ${type} cache? This action cannot be undone.`;
            if (confirm(confirmMessage)) {
                ...
                // Simulate cache clearing (you can implement actual AJAX call here)
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        if (type === 'files') btn.classList.add('btn-success');
                        if (type === 'database') btn.classList.add('btn-warning');
                    }, 2000);
                }, 1500);
            }
        }
        ...
        function optimizeSystem() {
            if (confirm('Run full system optimization? This may take a few minutes.')) {
                const btn = event.target;
                const originalText = btn.textContent;
        ...
        function cleanLogs() {
            if (confirm('Clean old log files? This will remove logs older than 30 days.')) {
                const btn = event.target;
                const originalText = btn.textContent;
        ...
        function refreshConfig() {
            if (confirm('Refresh configuration files? This will reload all system settings.')) {
                const btn = event.target;
                const originalText = btn.textContent;
